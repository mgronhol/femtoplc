...
class ReceiverThread( threading.Thread ):
	...
	def run( self ):
		while True:
			...
			values = struct.unpack_from( ">6B", data )
			
			for i in range( 8 ):
				b = (values[0] >> i) & 0x01
				self.app.ports["I0.%i"%i] = b
				b = (values[1] >> i) & 0x01
				self.app.ports["I1.%i"%i] = b
			
				b = (values[2] >> i) & 0x01
				self.app.ports["O0.%i"%i] = b
				b = (values[3] >> i) & 0x01
				self.app.ports["O1.%i"%i] = b
				
				b = (values[4] >> i) & 0x01
				self.app.ports["M0.%i"%i] = b
				b = (values[5] >> i) & 0x01
				self.app.ports["M1.%i"%i] = b
...
class MainWindow( wx.Frame ):
	...
	def draw_LD( self, dc, name, x, y ):
		...
		#print "LD", self.ports[name]
		
		if self.ports[name] == 1:
			dc.SetPen( wx.Pen('#00FF00') )
			dc.SetBrush( wx.Brush('#00FF00') )
			
			dc.DrawRectangle( x+16, y-10, 10, 20 )
	...
	def draw_ST( self, dc, name, x, y ):
		...
		if self.ports[name] == 1:
			...
			dc.DrawRectangle( x+12, y-10, 16, 20 )
	...
	def draw_N( self, dc, x, y ):
		...
		dc.DrawLine( x+30, y-10, x+30, y+10 )
		dc.DrawLine( x+30, y, x+40, y )
		dc.DrawLine( x+10, y-10, x+30, y-10 )
		dc.DrawLine( x+10, y+10, x+30, y+10 )
		dc.DrawLine( x+10, y+10, x+30, y-10 )
	
		dc.DrawText( "NOT", x+10, y-25 )
	
	def draw_CALL( self, dc, name, x, y ):
		
		#print "LD", self.ports[name]
		
		
		dc.DrawText( "CALL", x+10, y - 25 )
		...
		dc.DrawText( name, x+code_offset, y - 6 )
...
class EditorWindow( wx.Frame ):
	...
	def init_ui( self ):
		...
		self.text.SetKeyWords( 0, "ld ldn a brx brn st stn n call")
...
rthread = ReceiverThread( main, 7171 )
